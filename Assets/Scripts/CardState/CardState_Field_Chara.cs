...
public class CardState_Field_Chara : CardState_Field
{
    ...
    public override bool Enter()
    {
        if (Special)
        {
            ...
            //アニメーションを作成しEnqueue
            AnimationManager.I.AddSequence<AnimationManager.SpecialSummon>(() => AnimationMaker.SpecialSummon(Playable, Index)
            , "カードをプレイ");
        }
        else
        {
            //アニメーションを作成しEnqueue
            AnimationManager.I.AddSequence(() => AnimationMaker.PlayAnimation(Playable, Index, GameObject.Find("Canvas").transform)
            , "カードをプレイ");
        }
