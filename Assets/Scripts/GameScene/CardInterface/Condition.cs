...
public class Condition
{
    ...
    public List<StatusBase> DecideOption(StatusBase owner, List<StatusBase> playerCards, List<StatusBase> rivalCards)
    {
        ...
        ans = TargetParameter switch
        {
            ...
            Parameter.Atk => targetOption.ConvertType<IPlayable>().Where(c=>c.GetAtk()!=null).Where(c => comparison((int)c.GetAtk(), Threshold) > 0).ConvertType<StatusBase>(),
            Parameter.Hp => targetOption.ConvertType<IPlayable>().Where(c => c.GetHp() != null).Where(c => comparison((int)c.GetHp(), Threshold) > 0).ConvertType<StatusBase>(),
            _ => throw new NotImplementedException()
        };
