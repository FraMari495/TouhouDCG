...
public class PlayableIdManager : MonoSingleton<PlayableIdManager>
{
    ...
    /// <returns></returns>
    public PlayableId GetId(IPlayable playable,int? playableId)
    {
        sbyte modif = (sbyte)(ConnectionManager.Instance.IsFirstAttack ? 1 : -1);
    ...
    /// <param name="id"></param>
    /// <returns></returns>
    public IPlayable GetPlayableById(PlayableId id)
    {
        if (playableMap.ContainsKey((int)id))
        {
            return playableMap[(int)id];
        }
        else
        {
            Debug.LogError($"id{(int)id}のカードは存在しません");
